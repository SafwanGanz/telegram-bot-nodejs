...
const devloper = mdClient.db("MyBotDataDB").collection("devloper");

const makeDevloper = async (jid, set, role) => {
    const res = await devloper.findOne({ _id: jid });
    if (res == null) {
        await devloper.insertOne({
            _id: jid,
            isDev: set,
            role: role
        });
...
module.exports = { createMembersData, getMemberData, member, group, createGroupData, getGroupData, memberCount, devloper, makeDevloper };
